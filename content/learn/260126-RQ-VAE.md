...
1. 将原始数据通过编码器构建Embedding向量空间，然后进行latent降维Embedding（比如: 1024->64）
2. 进入到RA-VAE(残差量化)模块，会进行三层量化：
...
5. 生成的Semantic IDs如何映射为item？流程：离散编码→连续向量→具体内容，逆向还原。这儿需要用到Codebook，用Semantic ID查到对应的code vector，然后通过解码得到对应的item Embedding。


RQ-VAE量化的损失计算（主要是得到对应的每一个codebook）：
![20260126161521](https://coderethan-1327000741.cos.ap-chengdu.myqcloud.com/blog-pics/20260126161521.png)
...
参考资料：
- 原始论文TIGER：[Recommender Systems with Generative Retrieval](https://arxiv.org/pdf/2305.05065)
- 原始论文仓库代码：[RQ-VAE-Recommender](https://github.com/EdoardoBotta/RQ-VAE-Recommender)

待学习资料：
- https://zhuanlan.zhihu.com/p/1927502066894505349
- 残差量化沙漏现象：[Breaking the Hourglass Phenomenon of Residual Quantization: Enhancing the Upper Bound of Generative Retrieval](https://arxiv.org/abs/2407.21488)
- https://blog.csdn.net/zheli1969917/article/details/147482248
